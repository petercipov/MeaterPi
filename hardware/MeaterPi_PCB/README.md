...
The test uses BCM GPIO17 at 10 kHz, reads tach/RPM on BCM GPIO27, sweeps the requested fan speed from 100% down to 10%, requests 0% for 5 seconds, then sweeps back up in a loop. Each step prints measured RPM from the standard 2 tach pulses per revolution. Because Q1 inverts the signal, the script handles the inversion internally. The PC fan PWM target is normally 25 kHz, but `lgpio` rejected 15 kHz, 20 kHz, and 25 kHz on this Raspberry Pi setup with `bad PWM frequency`.
